...
import React, { useEffect, useState } from "react";
import { FaRegFilePdf, FaShareAlt, FaTrash } from "react-icons/fa"; // Import the trash icon
import { useSelector } from "react-redux";
import axios from "axios";
import Header from '../components/Header';
import Footer from "../components/Footer";
...
const Profile = () => {
  ...
  const handleDelete = async (fileId) => {
    try {
      ...
      await axios.delete(`${import.meta.env.VITE_API_URL}/notes/${fileId}`, {
        headers: {
          Authorization: `Bearer ${token}`
        }
      });
          ...
          <div className="grid grid-cols-1 gap-5 p-4 sm:grid-cols-2 md:grid-cols-3">
            {userFiles.map((file) => (
              <div key={file._id} className="relative mb-3 flex h-auto max-w-[300px] items-center justify-between gap-10 rounded-xl border border-black p-4 bg-buttoncolor text-textcolor">
                ...
                {/* Delete icon in the bottom right corner */}
                <button
                  className="absolute bottom-2 right-2 text-white"
                  onClick={() => handleDelete(file._id)} // Call function to delete the note
                >
                  <FaTrash size={20} />
                </button>
              </div>
